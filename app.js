...
const BEST_KEY = "organTrivia.bestStreak";
...
// Pick `n` pieces: the answer plus distinct distractors.
...
let current = null;      // the answer piece
...
function renderOptions(choices) {
  ...
  choices.forEach((piece, i) => {
    ...
    btn.innerHTML =
      ...
      `<span class="opt-composer"></span>`;
    btn.querySelector(".opt-title").textContent = piece.title;
    btn.querySelector(".opt-composer").textContent = piece.composer;
...
function choose(piece, btn) {
  ...
  if (correct) {
    ...
    stats.streak = 0;
    resultEl.textContent = `✗ It's ${current.title} — ${current.composer}`;
    resultEl.className = "result bad";
  }

  const parts = [];
  if (current.year) parts.push(current.year);
...
if (PIECES.length < 2) {
  scoreEl.textContent = "Add at least two pieces in data/pieces.js to play.";
